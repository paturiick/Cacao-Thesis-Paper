...
To ensure repeatable data collection and safe UAV operation, all field tests will be conducted only within a strictly defined controlled area inside the Janog Cacao Plantation. The exact block to be used (e.g., Block A: Rows R1–Rk) and the final parameters will be confirmed during the next site visit, since the plantation layout, obstacle locations, and link reliability conditions will be verified on-site; however, once selected, the same block and boundary limits will be fixed and used consistently for all succeeding missions.

<<<<<<< HEAD
To minimize blind spots and pod occlusion caused by leaves and branches, the UAV will implement multi-view coverage by alternating its facing direction across passes (e.g., right-facing on one pass and left-facing on the return pass), while maintaining a steady speed and controlled distance from the tree line. Smooth turns at row ends (U-turns) will be performed to transition to the next row segment while preserving consistent capture conditions.
\newpage
=======

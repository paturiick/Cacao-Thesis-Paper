...
The research setup will utilize a UAV (Unmanned Aerial Vehicle) equipped with a built-in high-definition camera and GPS module to conduct low-altitude monitoring of the cacao farm. The UAV will operate at approximately 1 meter above ground level to capture detailed visual information of cacao pods under the canopy, where early symptoms of black pod rot are more visible than in high-altitude passes.

The UAV will follow predefined flight paths along the corridors between cacao rows to ensure systematic coverage of the plantation. While the UAV moves parallel to each cacao row, it will maintain a yaw (heading) orientation such that the camera faces the cacao trees during recording. This side-facing strategy keeps pods consistently inside the camera field of view and reduces missed captures caused by limited viewpoints.
...
By flying parallel to the cacao rows while facing the trees, the UAV achieves overlapping visual coverage of pods, reduces missed detections due to occlusion, and supports stable real-time YOLO inference under actual farm conditions for detecting visible symptoms of \textit{Phytophthora palmivora}.
